...
public abstract class Magcard extends Utils.DialogFragment
{
	...
	String driver = prefs.getMagcardDeviceDriver();
	...
	if (driver.equalsIgnoreCase("virtual")) {
	    ...
	    magcard = new MagcardUSB(prefs.getMagcardDeviceNameOrVid(),
		    prefs.getMagcardDevicePathOrPid());
	} else if (driver.equalsIgnoreCase("serial")) {
	    magcard = new MagcardSerial(prefs.getMagcardDevicePathOrPid());
	} else {
	...
	task = new MagcardTask(manager, textview);
	task.execute();
    }

    public class MagcardTask extends AsyncTask<Void, Void, CharSequence>
    {
	private FragmentManager manager;
	private TextView textview;

	public MagcardTask(FragmentManager manager, TextView textview) {
	...
	protected void onPostExecute(CharSequence cardno) {
		...
		if (!(textview instanceof TextView)) {
		    LogActivity.writeLog("MagcardTask 参数必须是 TextView 实例");
		} else {
...
class MagcardUSB extends Magcard
{
    ...
    public MagcardUSB(String vid, String pid) {
	this.vid = vid;
	this.pid = pid;
    }
